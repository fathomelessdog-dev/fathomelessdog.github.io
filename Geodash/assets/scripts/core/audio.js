class AudioManager {
  constructor(scene) {
    ...
    this._audioLoadPromises = new Map();
  ...
  _getSongFileName(songKey) {
    if (!songKey || !window.allLevels) return null;
    const levelEntry = window.allLevels.find((level) => level[0] === songKey);
    if (!levelEntry) return null;
    return levelEntry[4] ? levelEntry[4] : levelEntry[1].replaceAll(" ", "");
  }
  _getMusicUrl(songKey) {
    if (songKey === "menu_music") return "assets/music/menuLoop.mp3";
    if (songKey === "StayInsideMe") return "assets/music/StayInsideMe.mp3";
    if (!songKey || songKey.startsWith("ng_song_")) return null;
    const fileName = this._getSongFileName(songKey);
    return fileName ? `assets/music/${fileName}.mp3` : null;
  }
  _loadAudio(key, url) {
    if (!key || !url) return Promise.resolve(false);
    if (this._scene.cache.audio.exists(key)) return Promise.resolve(true);
    if (this._audioLoadPromises.has(key)) return this._audioLoadPromises.get(key);
    const loader = this._scene.load;
    const promise = new Promise((resolve) => {
      const onComplete = () => {
        cleanup();
        resolve(this._scene.cache.audio.exists(key));
      };
      const onError = (file) => {
      ...
      this._audioLoadPromises.delete(key);
    });
    this._audioLoadPromises.set(key, promise);
    return promise;
  }
  _nextMusicRequestId() {
    this._musicRequestId += 1;
    return this._musicRequestId;
  }
  startMusic(StartPosOffset = 0) {
    ...
    const requestId = this._nextMusicRequestId();
    if (this._scene._practicedMode && this._scene._practicedMode.practiceMode) {
      ...
      const practiceUrl = this._getMusicUrl(practiceSongKey);
      if (!practiceUrl) {
        this._setupAnalyser();
        return;
      }
      this._loadAudio(practiceSongKey, practiceUrl).then((loaded) => {
        if (loaded && requestId === this._musicRequestId) playPractice();
      });
      return;
    }
    ...
    const songUrl = this._getMusicUrl(_songKey);
    if (!songUrl) {
      this._setupAnalyser();
      return;
    }
    this._loadAudio(_songKey, songUrl).then((loaded) => {
      if (loaded && requestId === this._musicRequestId) playSong();
    });
  }
  ...
  startMenuMusic() {
    ...
    if (this._scene.cache.audio.exists("menu_music")) {
      playMenu();
      return;
    }
    const menuUrl = this._getMusicUrl("menu_music");
    if (!menuUrl) {
      this._setupAnalyser();
      return;
    }
    this._loadAudio("menu_music", menuUrl).then((loaded) => {
      if (loaded && requestId === this._musicRequestId) playMenu();
    });
  }
  stopMusic() {
    this._nextMusicRequestId();
    if (this._music) {
      this._music.stop();
    }
  ...
  fadeInMusic(durationMillis = 1000) {
    ...
    const songUrl = this._getMusicUrl(songKey);
    if (!songUrl) {
      this._setupAnalyser();
      return;
    }
    this._loadAudio(songKey, songUrl).then((loaded) => {
      if (loaded && requestId === this._musicRequestId) playSong();
    });
  }
